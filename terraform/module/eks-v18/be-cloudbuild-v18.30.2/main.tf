...
locals {
  ...
  vpc_id     = var.vpc_id == null ? data.aws_vpc.stp-vpc-backend.id : var.vpc_id
  subnet_ids = length(var.subnet_ids) == 0 ? data.aws_subnets.stp-vpc-backend-private.ids : var.subnet_ids
}
...
locals {
  secretsmanager_name        = var.create ? "${var.environment}/data/${var.aws_account_name}/${local.name}/${var.cloudbuild_service}" : ""
  secretsmanager_description = "Vault secrets for ${local.name} cloudbuild"
}
...
locals {
  secretsmanager_eks_name        = var.create ? "${var.environment}/data/eks/${local.name}" : ""
