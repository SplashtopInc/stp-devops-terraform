...
locals {
  cb_worker_ami_owner_id = var.hardened_worker_ami_owner_id
  cb_worker_ami_linux    = var.hardened_worker_ami_linux
...
locals {
  others_name         = "others"
  others_kubelet_args = format("--kubelet-extra-args '%s'", var.cb_kubelet_args_others)
}

# wine
locals {
  wine_name         = "wine"
  wine_kubelet_args = format("--kubelet-extra-args '%s'", var.cb_kubelet_args_wine)
}

# devops
locals {
  devops_name         = "devops"
  devops_kubelet_args = format("--kubelet-extra-args '%s'", var.cb_kubelet_args_devops)
}
...
module "eks" {
  ...
  self_managed_node_groups = {
    ...
    others = {
      ...
      bootstrap_extra_args = local.others_kubelet_args

      pre_bootstrap_user_data = <<-EOT
      export CONTAINER_RUNTIME="containerd"
      export USE_MAX_PODS=false
      echo 'hfsplus' > /etc/modules-load.d/hfsplus.conf && /bin/systemctl restart systemd-modules-load.service
      EOT
      ...
      launch_template_name            = format("%s-self-managed", local.others_name)
      launch_template_use_name_prefix = true
      launch_template_description     = format("Self managed %s node group", local.others_name)
      ...
      mixed_instances_policy = {
        instances_distribution = {
          on_demand_base_capacity                  = var.cb_others_on_demand_base_capacity
      ...
      iam_role_name            = format("%s-self-managed-node-group-role", local.others_name)
      iam_role_use_name_prefix = false
      iam_role_description     = format("Self managed node group %s role", local.others_name)
      iam_role_tags = {
      ...
      security_group_name            = format("%s-node-group-sg", local.others_name)
      security_group_use_name_prefix = false
      security_group_description     = format("%s node group security group", local.others_name)
    ...
    wine = {
      ...
      ami_id               = data.aws_ami.cloudbuild_node_ami.id
      bootstrap_extra_args = local.wine_kubelet_args
      ...
      launch_template_name            = format("%s-self-managed", local.wine_name)
      launch_template_use_name_prefix = true
      launch_template_description     = format("Self managed %s node group", local.wine_name)
      ...
      create_iam_role          = true
      iam_role_name            = format("%s-self-managed-node-group-role", local.wine_name)
      iam_role_use_name_prefix = false
      iam_role_description     = format("Self managed node group %s role", local.wine_name)
      iam_role_tags = {
        Purpose = "Protector of the kubelet"
      }
      iam_role_additional_policies = [
      ...
      create_security_group          = true
      security_group_name            = format("%s-node-group-sg", local.wine_name)
      security_group_use_name_prefix = false
      security_group_description     = format("%s node group security group", local.wine_name)
      security_group_rules = {
        phoneOut = {
          description = "Hello CloudFlare"
          protocol    = "udp"
    ...
    devops = {
      ...
      ami_id               = data.aws_ami.cloudbuild_node_ami.id
      bootstrap_extra_args = local.devops_kubelet_args
